...
function comp({replace}) {
    const [map, setMap] = useState(() => new Array(7).fill(0).map(() => new Array(48).fill(0)))
    ...
    const clearState = () => {
        setMap(new Array(7).fill(0).map(() => new Array(48).fill(0)))
        replace([])
    }
    ...
    return (
        <>
            ...
            <Flex className={'bl1ccc br1ccc pd14'}>
                {map.filter(row => row.indexOf(2) !== -1).length ?
                    <Flex flex={1}>
